...
public class AnimatedProgressBar extends LinearLayout {
	 ...
	 * 
	 * @param context
	 *            is the context passed by the constructor
	 * @param attrs
	 *            is the attribute set passed by the constructor
	 */
	private void init(final Context context, AttributeSet attrs) {
		int backgroundColor = context.getResources().getColor(
				ResourceUtil.getColorId(context, "ssearch_browser_progressbar_bg"));
		mProgressColor = context.getResources().getColor(ResourceUtil.getColorId(context, "ssearch_theme_primary"));
		mBidirectionalAnimate = context.getResources().getBoolean(ResourceUtil.getColorId(context, "ssearch_theme_primary"));

		mBidirectionalAnimate = false;

		LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		inflater.inflate(ResourceUtil.getLayoutId(context, "ssearch_animated_progress_bar"), this, true);
	...
	/**
	 * sets the progress as an integer value between 0 and 100. Values above or
	 * below that interval will be adjusted to their nearest value within the
	 * interval, i.e. setting a value of 150 will have the effect of setting the
	 * progress to 100. You cannot trick us.
	 * 
	 * @param progress
	...
	public void setProgress(int progress) {
		...
		if (progress < mProgress && !mBidirectionalAnimate) { // if the we only
																// animate the
																// view in one
																// direction
			...
			// previous progress
			mDrawWidth = 0;
		} else if (progress == mProgress) { // we don't need to go any farther
		...
		final int deltaWidth = (mWidth * mProgress / 100) - mDrawWidth; // calculate
																		// amount
																		// the
																		// width
																		// has
	...
	/**
	 * private method used to create and run the animation used to change the
	 * progress
	 * 
	 * @param initialWidth
	 *            is the width at which the progress starts at
	 * @param maxWidth
	 *            is the maximum width (total width of the view)
	 * @param deltaWidth
	 *            is the amount by which the width of the progress view will
	 *            change
	 */
